  ...
  new SamlStrategy(
    {
      ...
      publicCert: config.saml.publicCert === undefined
        ? undefined
        : (function () {
            logger.error(`DEBUG: saml publicCert config: ${config.saml.publicCert}`)
            if (typeof config.saml.publicCert === 'string') {
              try {
                return [fs.readFileSync(config.saml.publicCert, 'utf-8')]
              } catch (e) {
                logger.error(`SAML public certificate: ${e.message}`)
              }
            } else if (Array.isArray(config.saml.publicCert)) {
              return config.saml.publicCert.map((certPath) => {
                try {
